    ...
    try:
        ...
        for output in macd_output:
            ...
            if output['histogram'] > 0:
                alert_states[alert_arg] = [True, None]
            else:
                alert_states[alert_arg] = [False, None]

            if output['macd'] > 0:
                alert_states[alert_arg][1] = True
            else:
                alert_states[alert_arg][1] = False
    ...
    while (True):
        ...
        try:
            ...
            for output in macd_output:
                ...
                alert_arg = str(output['time_bin'])
                if output['histogram'] > 0:
                    alert_states[alert_arg][0] = True
                else:
                    alert_states[alert_arg][0] = False
            ...
            for key in alert_states:
                ...
                logger.debug('alert_states[key][0]: ' + str(alert_states[key][0]))
                logger.debug('alert_states_last[key][0]: ' + str(alert_states_last[key][0]))
                logger.debug('alert_states[key][1]: ' + str(alert_states[key][1]))
                logger.debug('alert_states_last[key][1]: ' + str(alert_states_last[key][1]))
                if alert_states[key][0] != alert_states_last[key][0]:
                    ...
                    logger.debug('No changes with MACD-Signal crossover.')
                if alert_states[key][1] != alert_states_last[key][1]:
                    ...
                    logger.debug('No changes with MACD-Zero crossover.')
